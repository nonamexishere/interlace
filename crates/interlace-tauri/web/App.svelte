<script lang="ts">
  ...
  let peopleShell: {
    pane: () => { selectPerson: (id: number, append?: boolean, keepConversation?: boolean) => Promise<void>; openPersonAtMessage: (a: number, b: number, c?: string | null) => Promise<void>; ensureTlIndexVisible: (n: number) => void; closeCopyMenu: () => void; scrollToLatest: () => void; copySelected: () => void; persistLastRead: (index: number) => void; extendSelection: (n: number) => void } | undefined;
    filteredIds: () => number[];
  } | undefined = $state();
  ...
  function onKey(e: KeyboardEvent) {
    ...
    handleAppKey(e, {
      ...
      extendSelection: (n) => peopleShell?.pane()?.extendSelection(n),
