...
```
orchestrator  →  researcher  →  human confirms  →  test-author  →  impl  →  gates  →  reviewer  →  human merge
```

Skip researcher only when the GitHub issue already names the exact helper,
files, and must-IDs. For spikes the research note **is** the deliverable —
stop there.
...
| researcher | `prompts/researcher.md` | Read the clone; compare approaches; write `state/<slug>-research.md` | Patch source; write tests; invent policy |
| test-author | `prompts/test-author.md` | Write tests from the issue + research must-IDs + public API | Open parser/identity/search impl bodies; invent policy |
...
Producer ≠ verifier. The agent that writes tests is not the one that writes the impl. Researcher may open impl bodies; test-author must not.
...
- **Researcher** — not blinded. May open impl bodies so the approach comparison is real.
- **Yes (test-author)** — WhatsApp / Gmail / Contacts parsers, identity, search. Test-author may read `model.rs` and `lib.rs` signatures, not those impl files. `assert_blind.py` fails on `crates/interlace-core/src/**` impl paths in a test-author `IN.md`.
- **No (Tauri chrome)** — test-author still writes acceptance from the issue / research must-IDs only; impl still must not edit those tests.
...
```
pipeline/prompts/{orchestrator,researcher,test-author,impl,reviewer}.md
pipeline/state/<slug>-{IN,research,test-author,impl,review}.md
