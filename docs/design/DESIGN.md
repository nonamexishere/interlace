...
| Runtime | No HTTP **client crate**. Sandboxed `.app` **includes** `network.client` so WKWebView can load `tauri://localhost` (omit-client = blank window; measured). **Omit `network.server`.** CSP `connect-src` is IPC-only. |
...
| D8 | Network enforcement | **Phase 1:** cargo-deny bans `reqwest`/`hyper`/**`tokio` crate entirely** on core+cli. **Phase 2 Tauri:** `tokio` without `net`; still deny `reqwest`/`hyper`/updater/http plugin. Sandbox **omits `network.server`**. **`network.client` is required** so WKWebView can load `tauri://localhost` (measured: sandbox-only `.app` is a blank window). CSP still cannot `connect-src` the public internet. | Trust "we just won't call reqwest"; allow tokio-minus-net in Phase 1; refuse Tauri entirely; omit `network.client` too | Sync CLI has no reason to pull tokio; a sandboxed WKWebView without `network.client` cannot paint the local UI | omit-client wins only if we drop the sandbox or replace WKWebView |
...
**CSP** (tauri.conf): `default-src 'self'; img-src 'self' asset: data: cas:; media-src 'self' cas: data:; style-src 'self' 'unsafe-inline'; connect-src ipc: http://ipc.localhost https://ipc.localhost; frame-src data:; font-src 'self'`.
...
**Entitlements** (`Interlace.entitlements`): `app-sandbox`, `cs.allow-jit`, `network.client` (WKWebView local `tauri://` only — without it the `.app` is blank), user-selected files + archive bookmark. **Omit `network.server`.** No HTTP client crate; CSP `connect-src` is IPC-only.
